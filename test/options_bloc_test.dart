...
void main() {
  ...
  test(
    'selectOption updates state locally and does not call loadOptions',
    () async {
      // Initial load
      await optionsBloc.loadOptions();
      expect(mockOptionsRepository.getCurrentOptionsCalls, 1);
  ...
  test(
    'selectDate updates state locally and does not call loadOptions',
    () async {
      await optionsBloc.loadOptions();
      expect(mockOptionsRepository.getCurrentOptionsCalls, 1);
  ...
  test(
    'selectTimeRange updates state locally and does not call loadOptions',
    () async {
      await optionsBloc.loadOptions();
      expect(mockOptionsRepository.getCurrentOptionsCalls, 1);
  ...
  test('selectDate(null) clears selectedDate in state', () async {
    ...
    expect(
      (optionsBloc.state as LoadedOptionsState).options.selectedDate,
      someDate,
    );
  ...
  test('selectTimeRange(null) clears selectedTimeRange in state', () async {
    ...
    final someTime = DateTimeRange(
      start: DateTime(2023, 1, 1, 10),
      end: DateTime(2023, 1, 1, 11),
    );
    await optionsBloc.selectTimeRange(someTime);
    expect(
      (optionsBloc.state as LoadedOptionsState).options.selectedTimeRange,
      someTime,
    );
